...
SecurityFilterChain web(HttpSecurity http, RelyingPartyRegistrationRepository registrations) throws Exception {
    ...
    http
        ...
        .saml2Login(withDefaults())
        .addFilterBefore(new Saml2AssertingPartyLogoutResponseFilter(successHandler, logoutHandler), CsrfFilter.class)
...
private Filter logoutRequestFilter(RelyingPartyRegistrationResolver registrationResolver) { <2>
    ...
    return new Saml2RelyingPartyLogoutRequestFilter(logoutRequestResolver);
}
...
SecurityFilterChain web(HttpSecurity http, RelyingPartyRegistrationRepository registrations) throws Exception {
    ...
    http
        ...
        .saml2Login(withDefaults())
        .addFilterBefore(new Saml2AssertingPartyLogoutRequestFilter(successHandler, logoutHandler), CsrfFilter.class);
...
private LogoutSuccessHandler logoutSuccessHandler(RelyingPartyRegistrationResolver registrationResolver) { <3>
    ...
    return new Saml2RelyingPartyLogoutResponseSuccessHandler(logoutResponseResolver);
}
----
<1> - First, add your signing key to the `RelyingPartyRegistration` instance or to <<servlet-saml2login-rpr-duplicated,multiple instances>>
...
The other URLs is customizable in `Saml2AssertingPartyLogoutRequestFilter` and `Saml2AssertingPartyLogoutResponseFilter`.
...
[source,java]
----
Saml2AssertingPartyLogoutResponseFilter filter = new Saml2AssertingPartyLogoutResponseFilter(logoutSuccessHandler, logoutHandler);
